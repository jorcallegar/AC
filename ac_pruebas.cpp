...
int main( int argc, char *argv[] )
{
	...
	***********/
	...
	for(int z=0; z<s ; z++)
	{
    	...
    	for(x=2; x<n-2; x++)
    	{
        	for(y=2; y<n-2; y++)
        	{
            	...
            	D[z*(n-5)*(n-5)+x*(n-5)+y] = (float)1.0/(1.0+exp(-B[z][x][y]));
	...
	cout << "B[0][0][0]: " << D[0*(n-5)*(n-5)+0*(n-5)+0] << endl;
	cout << "B[1][1][1]: " << D[1*(n-5)*(n-5)+1*(n-5)+1] << endl;
    cout << "B[2][2][2]: " << D[2*(n-5)*(n-5)+2*(n-5)+2] << endl;
    cout << "B[3][3][3]: " << D[3*(n-5)*(n-5)+3*(n-5)+3] << endl;
